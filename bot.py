...
async def process_symbol(symbol, bot, cash_to_spend):
    try:
        ...
        if qty_held > 0:
            ...
            reason = ""
            if last_fast < last_slow: reason = "SMA Kesişimi"
            elif last_close <= sl_price: reason = "STOP LOSS"
            elif last_close >= tp_price: reason = "TAKE PROFIT"
            elif last_rsi > 85: reason = "RSI Asırı Alım"
        ...
        if qty_held == 0 and not was_sold and last_fast > last_slow:
            if last_rsi < RSI_OVERBOUGHT and cash_to_spend > 0:
...
async def main():
    ...
    try:
        ...
        # Bakiye eksideyse botu durdur
        if current_cash <= 0:
            print(f"🚨 BAKİYE EKSİDE (${current_cash}). İşlem yapılamaz.")
            return
        
        cash_to_spend = current_cash * TRADE_PCT
        for s in SYMBOLS:
